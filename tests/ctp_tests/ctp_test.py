...
class TestCTPMessage(unittest.TestCase):
    def test_invalid_types_give_error(self):
        ...
        self.assertRaises(TypeError, CTPMessage, invalid_msg_type, seqnum)
        self.assertRaises(TypeError, CTPMessage, valid_msg_type, seqnum, invalid_data)
        self.assertRaises(TypeError, CTPMessage, valid_msg_type, seqnum, valid_data, invalid_id_type)
        self.assertRaises(TypeError, CTPMessage, valid_msg_type, seqnum, valid_data, valid_cluster_id, invalid_id_type)
        self.assertRaises(TypeError, CTPMessage, valid_msg_type, seqnum, valid_data, invalid_id_type, valid_sender_id)
    ...
    def test_invalid_ids_give_error(self):
        ...
        self.assertRaises(ValueError, CTPMessage, valid_msg_type, seqnum, valid_data, invalid_id_short, valid_sender_id)
        self.assertRaises(ValueError, CTPMessage, valid_msg_type, seqnum, valid_data, invalid_id_long, valid_sender_id)
        self.assertRaises(ValueError, CTPMessage, valid_msg_type, seqnum, valid_data, valid_cluster_id, invalid_id_short)
        self.assertRaises(ValueError, CTPMessage, valid_msg_type, seqnum, valid_data, valid_cluster_id, invalid_id_long)
    ...
    def test_pack_and_unpack(self):
        ...
        message = CTPMessage(msg_type, 0, data, test_cluster_id, test_sender_id)
        packet = message.pack()
        expected_packet_b = struct.pack('!cI32s32s',
